...
$logHandler= new CLogFileHandler(DISCUZ_ROOT."./source/plugin/tom_pay/logs/[".date("Y-m-d")."]magapp_cron.log");
$log = Log::Init($logHandler, 15);

Log::DEBUG("[corn] start");

$last_order_time = TIMESTAMP - 3500;
$orderList = C::t('#tom_pay#tom_pay_order')->fetch_all_list(" AND payment='magapp_pay'  AND order_status=1 AND order_time > {$last_order_time} ","ORDER BY id DESC",0,10);

Log::DEBUG("[corn] list :" . json_encode(iconv_to_utf8($orderList)));

if(is_array($orderList) && !empty($orderList)){
    foreach ($orderList as $key => $value){
        if(!empty($value['mag_order_id'])){
            Log::DEBUG("[corn] list start :" . json_encode(iconv_to_utf8($value)));
            $corn_return = qf_curl($_G['siteurl'].'source/plugin/tom_pay/magappReturn.php?from=corn&mag_order_id='.$value['mag_order_id']);
